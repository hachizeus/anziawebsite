...
import { ChevronDown, ChevronUp, Info } from '../utils/icons.jsx';
...
const AnalysisDisplay = ({ analysis }) => {
  ...
  const components = {
    ...
    li: ({node, ordered, ...props}) => (
      ...
      >
        {!ordered && (
          <span className="inline-block w-1.5 h-1.5 rounded-full bg-primary-500 mt-3 mr-2 flex-shrink-0"></span>
        )}
    ...
    blockquote: ({node, ...props}) => (
      <motion.blockquote
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="border-l-4 border-primary-200 pl-4 my-4 text-gray-600 italic"
        {...props}
      />
    ),
    ...
    >
      <div className="flex justify-between items-center mb-3 sm:mb-4 pb-2 border-b border-gray-200">
        <h2 className="text-lg sm:text-xl font-semibold text-gray-800 flex items-center">
          <Info className="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2 flex-shrink-0 text-primary-600" />
          <span className="truncate">Expert Analysis</span>
        </h2>
        <button 
          onClick={() => setExpanded(!expanded)} 
          className="text-gray-500 hover:text-primary-600 transition-colors p-1 rounded-full hover:bg-gray-100 flex-shrink-0"
