...
import { Search, Home, MapPin, DollarSign, Building } from '../utils/icons.jsx';
...
const SearchForm = ({ onSearch, isLoading }) => {
  ...
  const popularCities = ['Nairobi', 'Mombasa', 'Kisumu', 'Nakuru', 'Eldoret', 'Thika'];
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
        <div className="p-2 bg-primary-100 rounded-lg mr-3 w-10 h-10 flex items-center justify-center">
      ...
      <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
        ...
        <div className="relative">
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 focus:outline-none transition-shadow text-sm sm:text-base"
              required
            />
            {activeField === 'city' && (
              <motion.div 
              ...
              >
                <p className="px-3 py-1 text-xs font-medium text-gray-500">Popular Cities in Kenya</p>
                <div className="mt-1 max-h-48 overflow-y-auto">
                  {popularCities.map((city) => (
                    <div
                      key={city}
                      onClick={() => handleCitySelect(city)}
                      className="px-3 py-2 hover:bg-primary-50 cursor-pointer text-gray-700 flex items-center"
                    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div>
            <label htmlFor="maxPrice" className="flex items-center text-sm font-medium text-gray-700 mb-1.5">
              <DollarSign className="w-4 h-4 mr-1.5 text-primary-600" />
              Maximum Price (KSh)
            </label>
            <div className="relative">
              <input
          ...
          <div>
            <label htmlFor="propertyType" className="flex items-center text-sm font-medium text-gray-700 mb-1.5">
              <Home className="w-4 h-4 mr-1.5 text-primary-600" />
              Property Type
            </label>
            <select
              id="propertyType"
              ...
              onChange={handleChange}
              className="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 focus:outline-none transition-shadow appearance-none text-sm sm:text-base"
            >
          ...
          <div>
            <label className="flex items-center text-sm font-medium text-gray-700 mb-2 sm:mb-4">
              <DollarSign className="w-4 h-4 mr-1.5 text-primary-600" />
              Price Range: KSh {searchParams.maxPrice.toLocaleString()}
            </label>
            <input
              type="range"
              min="5000"
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.99 }}
          disabled={isLoading}
          className="w-full mt-2 sm:mt-4 bg-gradient-to-r from-primary-500 to-primary-700 text-white py-3 sm:py-4 px-4 sm:px-6 rounded-lg hover:from-primary-600 hover:to-primary-800 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition-all duration-300 font-medium shadow-lg disabled:opacity-70"
        >
          {isLoading ? (
