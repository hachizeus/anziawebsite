...
async function testMongoCRUD() {
  try {
    ...
    const updateResult = await productsCollection.updateOne(
      { _id: productId },
      { $set: { price: 89.99, updated_at: new Date() } }
    );
    console.log('Update result:', updateResult);
    
    // Read the updated product
    console.log('\nReading updated product...');
...
async function runTests() {
  try {
    ...
    // Test MongoDB CRUD operations
    const mongoCRUDSuccess = await testMongoCRUD();
    ...
    console.log('MongoDB CRUD Operations:', mongoCRUDSuccess ? '✅ PASSED' : '❌ FAILED');
    ...
    // Overall result
    const allTestsPassed = mongoConnectionSuccess && mongoCRUDSuccess && imageKitConfigSuccess && imageKitUploadSuccess;
